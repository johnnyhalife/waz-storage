...
- Added Entities operations on Table service to query, get_one, insert, update, merge and delete.
- Added support for running against the storage development fabriq.
...
Azure Tables REST API isn't fully covered here (see TODO's for more information).
	...
	# retrieves all entities from a table (returns a maximum of 1,000 items at one time (more details here: http://msdn.microsoft.com/en-us/library/dd135718.aspx)
	...
	service.query_entity('customer_table', {:continuation_token => entities.continuation_token} )

	# retrieves all records that match with the specified query but only the first fifteen rows
	service.query_entity('customer_table', {:expression => "(PartitionKey eq 'customer') and (Age eq 23)", :top => 15} )
